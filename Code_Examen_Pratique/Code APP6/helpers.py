...
def pzmap1(z, p, title):
    ...
    ax.set_xlim(1.5*np.amin(np.real(rootslist)), 1.5*np.amax(np.real(rootslist)))
    ax.set_ylim(1.5*np.amin(np.imag(rootslist)), 1.5*np.amax(np.imag(rootslist)))
    return fig, ax
...
def bodeplot(b, a, wn, title):
    ...
    w, h = signal.freqs(b, a, worN=wn)  # calcul la réponse en fréquence du filtre (H(jw)), fréquence donnée en rad/sec
    f = w/(2*np.pi)
    mag = 20 * np.log10(np.abs(h))
    ph = np.unwrap(np.angle(h))  # calcul du déphasage en radians
    phlin = np.rad2deg(ph)  # déphasage en degrés
    fig, ax = bode1(f, mag, phlin, title)
    return mag, ph, w, f, fig, ax
...
def timepltmulti1(t, u, w, tout, yout, title):
    ...
    for i in range(len(w)):
        ax[i].plot(t, u[i], 'r', alpha=0.5, linewidth=1, label=f'Input {w[i]} Hz')
        ax[i].plot(tout[i], yout[i], 'k', linewidth=1.5, label=f'Output {w[i]} Hz')
